...
public class SuicaReaderLibrary {
	 ...
	 * @throws IOException
	 */
	public byte[] readWithoutEncryption(byte[] idm) throws IOException {
		final int size = 10;
		ByteArrayOutputStream command = new ByteArrayOutputStream(100);
		command.write(0);           // データの長さ（ダミー）
		command.write(0x06);        // コマンドコード: Read Without Encryption
	 ...
	 * @throws Exception
	 */
	public String parse(byte[] res, Context context) throws Exception {
		// res[0] = データ長
		// res[1] = 0x07
		// res[2〜9] = カードID
		...
		for (int i = 0; i < size; i++) {
			...
			SuicaDto suicaDto = dto.parse(res, 13 + i * 16, context);
